...
delete()
{
	...
	if [[ $deletionDate > $dateForSuppression ]]; then
		continue
	fi
...
((timePeriodInSeconds = fm_value * secondsPerDay))
nowInSeconds=$(date +%s)

if ((timePeriodInSeconds > nowInSeconds)); then
	timePeriodInSeconds=$nowInSeconds
fi

dateForSuppression=$(date -R -d "@$nowInSeconds")
dateForSuppression=$(date -d "$dateForSuppression - $timePeriodInSeconds seconds" "+%FT%T")
getTrashedFiles
